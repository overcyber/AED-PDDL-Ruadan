(define (domain network-enumeration)
  ...
  (:constants
    ...
    (:predicates
        ...
        (some_condition_for_open_port ?p - port ?h - host)
        ; Ccondição específica que determina se uma porta está aberta

        ; Asfalto. Tapa buracos 
        (host_exploited ?h - host)
        (exploitation_successful ?p - port ?h - host)
        (host_compromised ?h - host)


    )   

    ; Algumas inconsistencia lógica   
    ...
    (:action scan_port
        ...
        (and
            ...
            (port_open ?p ?h)  ; Principio da simplicidade XD 
            ;(when (some_condition_for_open_port ?p ?h) (port_open ?p ?h))
            ;(when (not (some_condition_for_open_port ?p ?h)) (port_closed ?p ?h))
        )
    )
    ; Ação para verificar um serviço em uma porta aberta
    (:action check_service
        :parameters (?s - service ?p - port ?h - host ?v - version_type)
    ...
    :parameters (?s - service ?v - version_type ?p - port ?h - host)
    ; requer que uma versão específica do serviço esteja associada a um host e que a vulnerabilidade dessa versão ainda não seja conhecida
    :precondition (and (service_version ?s ?v ?p ?h) 
                       (not (vulnerability_known ?v ?s)))
    :effect
    (and
        ; Verifica se existe uma vulnerabilidade conhecida para a versão
        ; Se sim, atualiza o estado de conhecimento sobre a vulnerabilidade
        (vulnerability_known ?v ?s)

        ; Se uma vulnerabilidade conhecida for encontrada, considera que o host foi explorado ou tentativa 
        (when (vulnerability_known ?v ?s)
            (host_exploited ?h))
    )
)
    ...
    (:action exploit_port
        ...
        (and
            ...
            ; novo predicado hipotético para determinar se uma tentativa de exploração foi bem-sucedida, tem problema deixar o preticado hipotético aqui ? 
            (when (exploitation_successful ?p ?h)
            (host_compromised ?h))
            ;predicado para marcar que um host foi comprometido após uma exploração bem-sucedida.
            ; Efeito condicional para determinar se a exploração foi bem-sucedida; Sem ideias ! 
        )
    )
)
